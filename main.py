...
def add_cafe():
    if request.method == "POST":
        try:
            ...
            return jsonify(error={"Bad Request": str(err)}), 400
        else:
            try:
...
def delete_cafe(cafe_id):
    ...
    if request.method == "DELETE":
        ...
        total_ids = db.session.query(Cafe).count()
        # Checking if the entered ID greater than DB ids
        if (int(cafe_id) > total_ids or int(cafe_id) < 1 or
                not db.session.query(db.exists().where(Cafe.id == cafe_id)).scalar()):
            return jsonify(error={"Not Found": f"Sorry! The Cafe with {cafe_id} ID was not found in database So, "
                                               f"it cannot be deleted."}), 404
        ...
        if api_key == request.args.get("api-key") or api_key == request.headers["api-key"]:
            ...
            try:
                ...
                return jsonify(error={"Not Found": f"Sorry! The cafe with ID {cafe_id} was not found in the "
                                                   f"database."}), 404
            else:
